    ...
    */
    function index() { 
	    ...
	    if ($this->ion_auth->logged_in() || $this->ion_auth->is_admin()) {
			...
			$data['userCategories'] = $this->calendar->category($user->username);  		
    ...
    public function resize() {
		...
		if ($this->ion_auth->logged_in() || $this->ion_auth->is_admin()) {
			...
			$data = array(
				...
				'daystart'      =>  strftime('%Y-%m-%d %H:%M:%S', $this->languages->langtotime($this->input->post('daystart'), $lang)),
				'dayend'        =>  strftime('%Y-%m-%d %H:%M:%S', $this->languages->langtotime($this->input->post('dayend'), $lang)),
			...
			if($endrecurring && $rec==1 || $rec==7 || $rec==14 || $rec==30 || $rec==365){
			 $start_date = $this->languages->langtotime($data['daystart'], $lang);
			 $end_date = $this->languages->langtotime($data['dayend'], $lang); 
			 
			 $length=(int)$end_date-$start_date;
			 list($y,$m,$d) = explode('-',$endrecurring);
			 ...
			 $step=3600*24*$rec;
				...
				if($rec==1 || $rec==7 || $rec==14 || $rec == 365) {
					$step=3600*24*$rec; 
					
					for($i=$start_date;$i<=$end_date;$i+=$step){	
					...
					for($i=$start_date;$i<=$end_date;$i+=3600*24*(int)date('t', mktime(0, 0, 0, (int)date('m',$i), 1, (int)date('y',$i)))){	
						$data['start']=date('Y-m-d H:i:s',$i);
						$data['end']=date('Y-m-d H:i:s',$i+$length);   
	...
	public function delete_event() {
		...
		if ($this->ion_auth->logged_in() || $this->ion_auth->is_admin()) {
			...
			if ($this->form_validation->run() === FALSE) { 
				...
				$recurring = $this->security->xss_clean($this->input->post('ic_event_recurring')); 
				$endrecurring = $this->security->xss_clean($this->input->post('ic_event_endrecurring'));  

				$rec=(int)$recurring;  				 
				if($endrecurring && $rec==1 || $rec==7 || $rec==14 || $rec==30 || $rec==365){ 
					$this->calendar->delete_events($eid);		
				}else{
