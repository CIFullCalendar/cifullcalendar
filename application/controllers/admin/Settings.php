...
class Settings extends CI_Controller {
	...
	function calendar_settings() {
	    ...
	    if ($this->ion_auth->is_admin()) { 
			...
			$data['weeknumberswithindays'] = $this->setting['cal_weeknumberswithindays'];   
			...
			if ($this->input->post('calendar_submit')) { 
				...
				if ($this->form_validation->run() === FALSE) {
					...
					$this->Setting_model->updateSetting('cal_weeknumberswithindays', $this->input->post('cal_weeknumberswithindays')); 
	...
	function theme() {
	    ...
	    if ($this->ion_auth->is_admin()) { 
			...
			$data['themes'] = glob($themepath . '*[!errors]*', GLOB_ONLYDIR);
