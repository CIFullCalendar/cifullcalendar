...
class Ion_auth
{
	...
	public function __construct()
	{
		...
		$this->load->helper('email'); //sirdre
		$this->load->helper('url');
 
		$this->load->library('session');
		$this->load->library('notify');  //sirdre
		$this->load->library('Languages'); 
		
		$this->load->model('ion_auth_model'); 
		$this->load->model('setting_model'); //sirdre
	...
	public function forgotten_password($identity)    //changed $email to $identity
	{
		...
		$identifier = (valid_email($identity)) ? $this->config->item('identity','ion_auth') : "username";  // sirdre
		  
		if ( $this->ion_auth_model->forgotten_password($identity, $identifier) )   //changed
		{ 
			// Get user information
		  ...
		  $user = $this->where($identifier, $identity)->where('active', 1)->users()->row();  // changed to get_user_by_identity from email
			...
			if ($user)
			{
				...
				else
				{
					...
					$this->notify->notify_forgotten_password('forgot_password', $user->email, $user->forgotten_password_code);  //sirdre 
	...
	public function forgotten_password_complete($code)
	{
		...
		if ($new_password)
		{
			...
			else
			{
				...
				$this->notify->notify_forgotten_password_complete('reset_password', $profile->email, $profile->username, $new_password);  //sirdre
	...
	public function register($identity, $password, $email, $additional_data = array(), $group_ids = array()) //need to test email activation
	{
		...
		else
		{
			...
			if(!$this->config->item('use_ci_email', 'ion_auth'))
			{
				...
				$this->notify->notify_register('registration', $user->username, $user->id, $email, $activation_code);  //sirdre
	...
	public function logout()
	{
		...
		else
		{
			...
			if (version_compare(PHP_VERSION, '7.0.0') >= 0) {  //agung-wete fix
				session_start();
			}			
	...
	public function logged_in()
	{
		...
		$this->ion_auth_model->update_last_page($this->get_user_id()); //sirdre
