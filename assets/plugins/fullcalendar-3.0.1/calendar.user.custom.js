...
;;
	...
	$.cifullCalendar = { version: "3.1.7.1" };  
   ...
   $.fn.cifullCalendar = function (option) {
	...
	var url = window.location.pathname.replace(regex, './home/');
		
	var regex = new RegExp("(.+)/index.php/"); 
	var seg = window.location.pathname.replace(regex, '');
	
	var username = new RegExp("index.php/(([a-z]+)|([0-9]+)|([a-z]+)([0-9]+))"); 
	var siteurl = window.location.pathname.replace(username, '') 
				+ "assets/attachments/";	
 
	var eventSources = new Array();
	var filterSources = new Array();  				
	
	eventSources[0] = 'home/ujson/'+seg;  
	realtime();
	...
	getRequest(url +"get_businessstart", function(err, data) {	 
		if(!empty(data)){businessStart = data;} 	
	});	
