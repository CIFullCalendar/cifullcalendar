...
;;
   ...
   $.fn.cifullCalendar = function (option) {
	...
	var get_url = window.location.pathname.replace(regex, './index.php/home/'); 
	...
	getRequest(get_url +"get_lang", function(err, data) {
		...
		$('#calendar').fullCalendar({
			...
			eventClick: function(event, jsEvent, view)  { 
				...
				var attach = (event.filename) ? '<a href="data:application/octet-stream;base64,' + event.filedata + '" download="' + event.filename + '" ><b>' + event.filename + '</b></a>' : '';	
				...
				$('#viewEventModal #Iexport').html("<a href='"+ get_url +"export/" +
				...
				$('#viewEventModal #ic_event_allday').text(event.alldayText);  
				$('#viewEventModal #filename').html(attach);	 
	...
	function updateClock(){
		...
		var zoneAbbr = now.zoneAbbr();
		var digiclock = "<i class='fa fa-clock-o'></i> " + now.format(clockformat);
		var timezone = "<i class='fa fa-globe'></i> " + currentTimezoneCode2;		
