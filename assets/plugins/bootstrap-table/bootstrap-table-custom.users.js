...
;; 	    
	...
	function authFormatter(value, row) {
        var icon = row.auth == 0 ? 'fa-toggle-off' : 'fa-toggle-on'
        var name = row.auth == 0 ? 'public' : 'private' 
		
        return '<i class="fa ' + icon + '" ></i>' + name;
    }	
	
	function groupsFormatter(value, row, index) {	
		var total = row.groups;
		var g = new Array();
        $.each(total, function (i, data) {
            g += ' <a class="btn btn-default btn-xs" role="button" >'+ data.name + '</a>';
        }); 
		return g;
	}	
	...
	function editFormatter(value, row, index) {		 		
		return [
			'<a href="userslist/edit/'+ row.id +'" class="btn btn-primary btn-xs" role="button" ><i class="fa fa-pencil"></i></a>'
		].join('');
	}