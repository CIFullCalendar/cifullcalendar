...
var allpages_url = window.location.protocol + "//" 
...
var site_url = window.location.protocol + "//" 
			+ window.location.host
			+ window.location.pathname.replace('/index.php/admin/pages', '')
			+ "/page/title/";	
var $table = $('#pages_dataTable'),
	$remove = $('#remove-data'),
	selections = [];
...
;; 	  	   
    ...
    function totalFormatter(data) {
        ...
        $.each(data, function (i, row) {
            total += +(row.id.substring(1));
        });
        return total;
    }
	
	function timestampFormatter(value, row) { 
		var now = moment(row.pubdates); 
	...
	function operateFormatter(value, row, index) {		 		
		return [
			'<a href="'+ site_url +''+ row.seo +'" class="btn btn-success btn-xs" role="button" ><i class="fa fa-search-plus"></i></a>',
			'<a href="pages/edit/'+ row.id +'" class="btn btn-primary btn-xs" role="button" ><i class="fa fa-pencil"></i></a>',
